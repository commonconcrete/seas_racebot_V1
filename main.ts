/**
 * Joystick:Bit Haptics functions event handlers
 */
/**
 * Joystick:Bit background task that checks for press of buttons C, D, E, F and transmits relevant messages to Bot.
 */
/**
 * joystick:bit background task. Continuously polls joystick (rocker) position and sends radio messages to Bot.
 */
/**
 * Joystick:Bit A & B Button event handlers
 */
// joystick:bit background task. Continuously polls controller buttons C, D, E, & F and sends radio messages to Bot.
...
/**
 * Change led brightness. Not called by any part of this code set.
 */
...
// Helper Function to clear the LED matrix
...
/**
 * Helper Functions to control wuKong LED lights and siren volume
 */
function Turn_Flashers_On () {
    music.setVolume(255)
    Police_Lights_On = true
    strip.showColor(Light_Color)
    strip.show()
}
...
/**
 * Bot functions triggered by radio messages received from joystick:bit controller. These functions are called in series by the "on radio received" event handler.
 */
// Perform basic motor functions as commanded by joystick rocker.
...
// Perform specialized Servo Arms functions as commanded by controller haptics.
...
// User initialization code for peripherals. (No function in this rev.
...
// Joystick:Bit Haptic motion event handlers
...
/**
 * Initialization code for peripherals (servos, etc.)
 */
...
/**
 * Helper Function to clear the LED matrix. Called by Button A and Button B handlers
 */
...
/**
 * Bot event handler for radio messages received from joystick:bit controller
 */
// Handler for Bot radio message received event.
...
/**
 * Initialize variables and constants. Called by "On Start"
 */
...
// SEAS ICSC swimlane bot code to operate police lights & siren during forward travel.
function Turn_Flashers_Off () {
    music.setVolume(0)
...
network_GroupChannel_MyBotAndController_Base0_Int = 43
setup_For_System_Func()
setup_For_Teacher_Func()
quest_Note_2.quest_Show_String_For_Note_Small_Func(
"Initialize Police Flashers State"
)
strip = neopixel.create(DigitalPin.P16, 4, NeoPixelMode.RGB)
Light_Color = neopixel.colors(NeoPixelColors.Red)
Turn_Flashers_Off()
/**
 * Bot background task that implements police lights and sirens using wuKong's built-in rainbow LEDs and the micro:BIT's buzzer.
 * 
 * (SEAS robot for ICSC)
 */
// Update police lights and siren tone every 500ms
basic.forever(function () {
    quest_Note_2.quest_Show_String_For_Note_Big_Func(
    "Alternates flasher color and siren tone at 1/2 Hz"
    )
    if (Police_Lights_On) {
        strip.showColor(Light_Color)
        strip.show()
        ...
        if (Light_Color == neopixel.colors(NeoPixelColors.Red)) {
            music.play(music.stringPlayable("C5 - - - - - - - ", 115), music.PlaybackMode.InBackground)
            Light_Color = neopixel.colors(NeoPixelColors.Blue)
        } else {
            music.play(music.stringPlayable("G - - - - - - - ", 115), music.PlaybackMode.InBackground)
            Light_Color = neopixel.colors(NeoPixelColors.Red)
        }
    }
    basic.pause(500)
})
// This Joystick:Bit background task responds to presses of the C,D,E,F buttons.
basic.forever(function () {
    if (true) {
        quest_Note_3.quest_Show_String_For_Note_Big_Func(
...
// Background task to poll joystick motion and transmit fwd/back/left/right radio messages to bot.
basic.forever(function () {
    if (true) {
        quest_Note_3.quest_Show_String_For_Note_Big_Func(
...
/**
 * These two blocks are disabled Built-In Test for hardware elements.
 */
basic.forever(function () {
    quest_Note_6.quest_Show_String_For_Note_Big_Func(
    ""
...
/**
 * These two blocks are non-functional sets of comments apparently to do with a web server feature.
 */
basic.forever(function () {
    if (false) {
        quest_Note_6.quest_Show_String_For_Note_Big_Func(
...
// Disabled motor diagnostic/Built-In Test for Servos
basic.forever(function () {
    quest_Note_6.quest_Show_String_For_Note_Big_Func(
    "'On Logo Pressed'"
    )
...
/**
 * Coding notes and disabled hourly state display.
 */
// Disabled hourly task to plot state indications on the LED array.
